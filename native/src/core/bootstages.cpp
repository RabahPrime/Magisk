...
void mount_mirrors() {
    ...
    xmount("worker", worker_dir.data(), "tmpfs", 0, nullptr);
    ...
    // remount /data with same flags
    struct statfs stfs{};
    statfs("/data", &stfs);
    xmount(nullptr, string(MAGISKTMP + "/" MIRRDIR "/data").data(), nullptr, MS_REMOUNT | (stfs.f_flags & ~MS_NOSUID), nullptr);
    ...
    if (access(SECURE_DIR, F_OK) == 0 || SDK_INT < 24) {
        ...
        if (!xmount(src.data(), dest.data(), nullptr, MS_BIND, nullptr)) {
            restorecon();
