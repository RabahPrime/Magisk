...
bool mount_mirror(const std::string_view from, const std::string_view to) {
...
void mount_mirrors() {
    ...
    do {
        ...
        for (auto &info : reversed(current_mount_info)){
            for (int i = 0; include_parts[i]; i++){
                if (info.target == include_parts[i]) {
                    string mirror_path = mirror_dir + include_parts[i];
                    mkdirs(mirror_path.data(), 0);
                    goto add_mount_list;
                }
                if (info.target.starts_with(std::string(include_parts[i]) + "/")){
                    goto add_mount_list;
                }
                continue;
                ...
                for (auto &s : mountpoint) {
                    ...
                    //   /a
                    //   /a/b
                    if (s == info.target || info.target.starts_with(s + "/"))
        ...
        if (std::find(mountpoint.begin(), mountpoint.end(), "/persist") == mountpoint.end()) {
            ...
            mkdirs(mirror_path.data(), 0);
            if (mount_mirror("/mnt/vendor/persist", mirror_path))
                LOGD("mount: %s\n", mirror_path.data());
        }
    } while(false);
    ...
    for (auto &info : reversed(mount_info)) {
        ...
        struct stat st{};
        string mirror_path = mirror_dir + mnt_dir;
        if (lstat(mnt_dir, &st) != 0 ||
            ...
            info.type == "vfat"sv)
                continue;
        if (mount_mirror(mnt_dir, mirror_path))
            LOGD("mount: %s\n", mirror_path.data());
    }
