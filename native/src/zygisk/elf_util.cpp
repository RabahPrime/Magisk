...
ElfImg::ElfImg(std::string_view base_name) : elf(base_name) {
    ...
    if (fd < 0) {
        ZLOGE("failed to open %s\n", elf.data());
        return;
    }

    size = lseek(fd, 0, SEEK_END);
    if (size <= 0) {
        ZLOGE("lseek() failed for %s\n", elf.data());
    }
...
std::vector<ElfW(Addr)> ElfImg::LinearRangeLookup(std::string_view name) const {
    ...
    for (auto [i, end] = symtabs_.equal_range(name); i != end; ++i) {
        ZLOGD("found %s %p in %s in symtab by linear range lookup\n", name.data(),
             reinterpret_cast<void *>(i->second->st_value), elf.data());
        res.emplace_back(i->second->st_value);
    }
    return res;
}
...
ElfW(Addr) ElfImg::PrefixLookupFirst(std::string_view prefix) const {
    ...
    if (auto i = symtabs_.lower_bound(prefix); i != symtabs_.end() && i->first.starts_with(prefix)) {
        ZLOGD("found prefix %s of %s %p in %s in symtab by linear lookup\n", prefix.data(), i->first.data(), reinterpret_cast<void*>(i->second->st_value), elf.data());
        return i->second->st_value;
    } else {
        return 0;
    }
...
ElfImg::getSymbOffset(std::string_view name, uint32_t gnu_hash, uint32_t elf_hash) const {
    if (auto offset = GnuLookup(name, gnu_hash); offset > 0) {
        ZLOGD("found %s %p in %s in dynsym by gnuhash\n", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else if (offset = ElfLookup(name, elf_hash); offset > 0) {
        ZLOGD("found %s %p in %s in dynsym by elfhash\n", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else if (offset = LinearLookup(name); offset > 0) {
        ZLOGD("found %s %p in %s in symtab by linear lookup\n", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else {
        return 0;
...
bool ElfImg::findModuleBase() {
    ...
    if (!found) {
        if (buff) free(buff);
        ZLOGE("failed to read load address for %s\n", elf.data());
        fclose(maps);
        return false;
    }
