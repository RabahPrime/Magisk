...
void revert_unmount(int pid) {
    ...
    for (auto &info : parse_mount_info("self")) {
        if ((info.target.starts_with(MAGISKTMP) ||
             info.root.starts_with("/" INTLROOT "/") ||
             info.source.starts_with(MAGISKTMP)) &&
    ...
    // unmount module node
    for (auto &info : parse_mount_info("self")) {
        if (data_stat_ret == 0 && info.device == data_stat.st_dev 
